...
void cmd_callback(const upros_message::TagCommand::ConstPtr &msg)
{
    ...
    ROS_INFO("Received target tag: %d", target_tag);
}
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("Received valid command, starting grab sequence...");

    std::string tag_link;
    if(target_tag == 1)
    {
        tag_link = "tag_1";
        ROS_INFO("Target selected: tag_1");
    }
    else if (target_tag == 2)
    {
        tag_link = "tag_2";
        ROS_INFO("Target selected: tag_2");
    }
    ...
    geometry_msgs::TransformStamped tfs_1 = buffer.lookupTransform("base_link", tag_link, ros::Time(0), ros::Duration(100));
    ...
    ROS_INFO("TF transform received!");
    ...
    ROS_INFO("Moving arm to target position...");
    arm_move_open_client.call(move_srv);
    sleep(5.0);
    ...
    ROS_INFO("Grab task completed successfully!");
